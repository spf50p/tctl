...
- `.tctl.yaml` — config (default; override with `-c/--conf`). Fields: `collect_file_path`, `aggregate_file_path`, `days_file_path` (all support `strftime` placeholders; `days_file_path` defaults to `/var/lib/tctl/days.json` when empty), `mmdb_city` (required for `aggregate`), `mmdb_asn`, `mmdb_country` (declared but not yet consumed), `telemt_servers` (list of `{base_url, token}`).
